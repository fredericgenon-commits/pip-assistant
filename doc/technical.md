...
Fetches DEV child stories per REQ, computes remaining story points and Team Status per
team, and persists results. Follows the hexagonal pattern.

### Domain

- **`DevTicket`** (record) — `key`, `summary`, `status`, `deliveryMethod`, `jiraTeam`,
  `storyPoints (Integer)` — value object returned by the JIRA adapter.
- **`BacklogCalculator`** (pure domain service, not a Spring bean — instantiated with
  `new BacklogCalculator()`) — given a list of `DevTicket` and a
  `Map<String,Long> jiraTeamToId`, produces `Map<Long, TeamBacklogResult>` (SP + Team
  Status per team).
  - Only tickets with `deliveryMethod == "project"` (case-insensitive) are counted.
  - Non-mapped `jiraTeam` values are ignored.
  - **TA detection**: summary starts with `[technical analysis]`, `[technical_analysis]`,
    `[technical-analysis]`, or `[tech. analysis]` (case-insensitive).
  - **Team Status priority** (first match wins): TA non-abandoned open/tbe/rfi → *TA
    todo*; TA in-progress → *TA ongoing*; DEV non-TA non-abandoned tbe → *To be
    estimated*; all DEV non-abandoned rfi → *Ready*; all DEV non-abandoned done →
    *Done*; else null.
  - Backlog SP = sum of SP for non-TA, non-abandoned, RFI tickets of known team.
- **`JiraPort`** — `Optional<String> fetchStatus(String issueKey)`.
- **`JiraBacklogPort`** — `List<DevTicket> fetchDevTickets(String reqKey)`.
- **`RequirementBacklogRepository`** — `upsertTeamStatus`, `findByPip` (returns
  `List<TeamBacklogEntry>`).

### Application

- **`JiraSyncService`** — orchestrates one sync pass for a PIP:
  1. TTL check — `ConcurrentHashMap<Long, Instant>`; skips if last sync < 10 min ago.
  2. For each requirement: `jiraPort.fetchStatus` → update status; `jiraBacklogPort.fetchDevTickets` → `BacklogCalculator.compute`.
  3. SP > 0 for a (req, team) → `upsertWorkloadFromJira` (sets estimate, `jira_locked=true`, `manual_override=true`).
  4. SP == 0 and was locked → `unlockWorkloadFromJira` (`jira_locked=false`, `manual_override=false`).
  5. `upsertTeamStatus` for every team (null when no status applies).
  6. Returns `JiraSyncResult(synced, failed, errors)`.

### Infrastructure

- **`JiraProperties`** (`@ConfigurationProperties("pip.jira")`) — `baseUrl`, `apiToken`,
  `syncTtlMinutes` (10), `interactionSyncThresholdSeconds` (60), nested `Fields`
  (custom field IDs for delivery-method, team, story-points), `Map<String,String>
  teamMapping` (e.g. `"Core Team"` → `"Core"`).
- **`MockJiraAdapter`** (`@Profile("jira-mock")`) — deterministic fake data: 2 DEV Core
  RFI 3 SP, 1 TA Core Open, 1 DEV Portal TBE 5 SP, 1 DEV Process Done. Used for local
  development without a real JIRA instance (`SPRING_PROFILES_ACTIVE=jira-mock`).
- **`JiraClientAdapter`** (`@Profile("!jira-mock")`) — real adapter using `RestClient`
  with Bearer token.
  - `fetchStatus`: `GET /rest/api/2/issue/{key}?fields=status`.
  - `fetchDevTickets`: `GET /rest/api/2/search?jql=issueType=Story AND "Epic Link"={reqKey}&fields=…&maxResults=100`.
- **`RequirementBacklogRepositoryAdapter`** — JPA adapter; `findByPip` uses a JPQL
  query joining `requirement → project → pip`.
- **`JiraSyncController`**:
  - `POST /api/pips/{id}/sync` → `JiraSyncResult`.
  - `GET /api/jira-sync-settings` → `{ interactionThresholdSeconds }`.
...
`PipDetail` constructor:

1. Calls `getSyncSettings()` once to store the interaction threshold.
2. Fires `syncJira()` immediately (first sync on load).
3. Schedules `interval(10 * 60 * 1000).pipe(takeUntilDestroyed(...))` for periodic syncs.

`@HostListener('document:click')` / `'document:keydown'` → `syncJira()` if more than
...
sequenceDiagram
    ...
    loop per requirement
        ...
        J-->>S: status + List<DevTicket>
        S->>S: BacklogCalculator.compute(tickets, teamMap)
        S->>D: upsertWorkloadFromJira / unlockWorkloadFromJira / upsertTeamStatus
    end
